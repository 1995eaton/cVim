...
(function($) {
    ...
    let animationYFrame, animationXFrame,
        scrollXFunction, scrollYFunction,
        scrollElem, scrollXElem,
        lastX, lastY;
    let holdKeyScroll = false;

    let easeFn = function(t, b, c, d) {
        return (t === d) ? b + c : c * (-Math.pow(2, -10 * t / d) + 1) + b;
    };

    let timeFn = typeof window.performance === 'undefined' ?
    ...
    scrollYFunction = function() {
        let delta = easeFn(scroll.ty, scroll.y0, scroll.y1 - scroll.y0, scroll.dy);
        let time = timeFn();
        scroll.yc = delta;
    ...
    scrollXFunction = function() {
        let delta = easeFn(scrollx.tx, scrollx.x0, scrollx.x1 - scrollx.x0, scrollx.dx);
        let time = timeFn();
        scrollx.xc = delta;
    ...
    let holdFunction = function(dx, dy) {
        let se = dx ? scrollXElem : scrollElem;
    ...
    $.smoothScrollBy = function(elem, x, y, d) {
        ...
        holdKeyScroll = false;
        if (x) {
            let oldDx = scrollx.x1 - scrollx.xc;
            $.cancelAnimationFrame(animationXFrame);
            scrollx.dx = d;
            scrollx.x0 = scrollXElem.scrollLeft;
...
Scroll.scroll = function(type, repeats) {
    ...
    let stepSize = settings ? settings.scrollstep : 60;
    ...
    let scrollElem = scrollingElement(direction),
